...
export async function GET(request: Request) {
  try {
    ...
    await tryMatchTetrisPlayers();

    // Check if still in queue
    const queueStatus = await getTetrisQueuePosition(user.id);
    ...
    // Not in queue - check if matched to a Tetris game
    ...
    if (recentMatch && recentMatch.status === "in_progress") {
      await processGravityDrops(recentMatch.id);
